...
if __name__ == "__main__":
    ...
    # should be able to pop two out
    assert test_Stack.isEmpty() == True, "previous code didn't clear out test_Stack"
    test_Stack.push("test1")
    test_Stack.push("test2")
    assert test_Stack.isEmpty() == False, "push failed"
    result1 = test_Stack.pop()
    result2 = test_Stack.pop()
    assert test_Stack.isEmpty() == True, "pop didn't remove data as expected"

    # Test LiFo feature of stack
    new_item = item("james")
    test_Stack.push(new_item)
    assert test_Stack.peek() == new_item
    assert test_Stack.pop() == new_item
